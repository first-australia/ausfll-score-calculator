...
const validate = (answers: ScoreAnswer[]) => {
  ...
  if (answer(answers, 'm03a') === '0' && answer(answers, 'm03b') === 'yes')
    errors.push({
      id: 'm03a,m03b',
      message: "Mission 3 - Slide figures can't be on the slide and in base",
    });
  if (answer(answers, 'm03a') === '0' && answer(answers, 'm03c') === 'yes')
    errors.push({
      id: 'm03a,m03c',
      message:
  ...
  if (
    answer(answers, 'm03a') === '1' &&
    answer(answers, 'm03b') === 'yes' &&
    answer(answers, 'm03c') === 'yes'
  )
    errors.push({
      id: 'm03a,m03b,m03c',
  ...
  if (Number(answer(answers, 'm08b')) < 0)
    errors.push({
      id: 'm08b',
      message: 'Mission 8 - Negative number of cubes in the target',
    });
  if (Number(answer(answers, 'm08b')) > 17)
    errors.push({
      id: 'm08b',
      message: 'Mission 8 - Too many cubes in target',
    });
  if (
    answer(answers, 'm08c') === 'yes' &&
    Number(answer(answers, 'm08b')) === 0
  )
    errors.push({
      id: 'm08b,m08c',
      message:
  ...
  if (answer(answers, 'm12a') === 'no' && answer(answers, 'm12b') === 'yes')
    errors.push({
      id: 'm12a,m12b',
  ...
  if (Number(answer(answers, 'm14b')) < 0)
    errors.push({
      id: 'm14b',
      message: 'Mission 14 - Negative number of health units',
    });
  if (Number(answer(answers, 'm14b')) > 8)
    errors.push({
      id: 'm14b',
...
const score = (answers: ScoreAnswer[]) => {
  let _score = answer(answers, 'advg') === 'yes' ? 25 : 0;

  // Mission 1
  if (answer(answers, 'm01b') === 'rePLAY_logo') _score += 20;
  if (answer(answers, 'm01b') === 'bench_gray_area') _score += 20;

  // Mission 2
  _score += answer(answers, 'm02a') === 'magenta' ? 10 : 0;
  _score += answer(answers, 'm02a') === 'yellow' ? 15 : 0;
  _score += answer(answers, 'm02a') === 'blue' ? 20 : 0;

  // Mission 3
  _score += answer(answers, 'm03a') === '1' ? 5 : 0;
  _score += answer(answers, 'm03a') === '2' ? 20 : 0;
  _score += answer(answers, 'm03b') === 'yes' ? 10 : 0;
  _score += answer(answers, 'm03c') === 'yes' ? 20 : 0;

  // Mission 4
  _score += answer(answers, 'm04a') === 'yes' ? 10 : 0;
  _score += Number(answer(answers, 'm04b')) * 10;
  _score += answer(answers, 'm04c') === 'yes' ? 15 : 0;

  // Mission 5
  _score += answer(answers, 'm05a') === 'yes' ? 15 : 0;
  _score += answer(answers, 'm05b') === 'middle' ? 15 : 0;
  _score += answer(answers, 'm05b') === 'top' ? 25 : 0;

  // Mission 6
  _score += answer(answers, 'm06a') === 'yes' ? 15 : 0;
  _score += answer(answers, 'm06b') === 'yes' ? 30 : 0;

  // Mission 7
  _score += answer(answers, 'm07a') === 'yes' ? 20 : 0;

  // Mission 8
  if (answer(answers, 'm08d') !== 'yes') {
    _score += answer(answers, 'm08a') === 'yes' ? 25 : 0;
    _score += Math.max(Math.min(Number(answer(answers, 'm08b')), 17), 0) * 5;
    _score += answer(answers, 'm08c') === 'yes' ? 10 : 0;
  }

  // Mission 9
  if (answer(answers, 'm09c') === 'yes') {
    _score += answer(answers, 'm09a') === 'blue' ? 10 : 0;
    _score += answer(answers, 'm09a') === 'black' ? 0 : 0;
    _score += answer(answers, 'm09a') === 'both' ? 10 : 0;
    if (['blue', 'both'].includes(`${answer(answers, 'm09a')}` ?? '')) {
      _score += answer(answers, 'm09b') === 'blue' ? 5 : 0;
      _score += answer(answers, 'm09b') === 'both' ? 5 : 0;
    }
  } else {
    _score += answer(answers, 'm09a') === 'blue' ? 10 : 0;
    _score += answer(answers, 'm09a') === 'black' ? 15 : 0;
    _score += answer(answers, 'm09a') === 'both' ? 25 : 0;
    if (['blue', 'both'].includes(`${answer(answers, 'm09a')}`)) {
      _score += answer(answers, 'm09b') === 'blue' ? 5 : 0;
      _score += answer(answers, 'm09b') === 'both' ? 5 : 0;
    }
    if (['black', 'both'].includes(`${answer(answers, 'm09a')}`)) {
      _score += answer(answers, 'm09b') === 'black' ? 5 : 0;
      _score += answer(answers, 'm09b') === 'both' ? 5 : 0;
    }
  }

  // Mission 10
  _score += answer(answers, 'm10a') === 'yes' ? 15 : 0;
  ...
  // Mission 12
  _score += answer(answers, 'm12a') === 'yes' ? 15 : 0;
  _score += answer(answers, 'm12b') === 'yes' ? 15 : 0;

  // Mission 13
  _score += answer(answers, 'm13a') === 'blue' ? 10 : 0;
  _score += answer(answers, 'm13a') === 'magenta' ? 15 : 0;
  _score += answer(answers, 'm13a') === 'yellow' ? 20 : 0;

  // Mission 14
  _score += Math.max(0, Math.min(Number(answer(answers, 'm14a')), 8)) * 5;
  _score += Number(answer(answers, 'm14b')) * 10;

  // Mission 15
  switch (Number(answer(answers, 'm15a'))) {
    case 6:
      _score += 60;
      break;
    case 5:
