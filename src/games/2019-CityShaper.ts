...
const validate = (answers: ScoreAnswer[]) => {
  ...
  if (
    answer(answers, 'm01a') !== 'yes' &&
    Number(answer(answers, 'm01b')) !== 0
  )
    errors.push({
      id: 'm01a,m01b',
  ...
  if (Number(answer(answers, 'm05a')) + Number(answer(answers, 'm05b')) > 17)
    errors.push({
      id: 'm05a,m05b',
  ...
  if (Number(answer(answers, 'm12b')) < Number(answer(answers, 'm12a')))
    errors.push({
      id: 'm12a,m12b',
      message:
...
const score = (answers: ScoreAnswer[]): number => {
  ...
  const adv = answers.find((r) => r.id === 'advg')?.answer === 'yes' ? 5 : 0;

  // Score 1
  let pre = _score;
  if (answer(answers, 'm01a') === 'yes') _score += 20;
  _score += Number(answer(answers, 'm01b')) * 15;
  if (_score > pre) _score += adv;

  // Score 2
  pre = _score;
  _score += answer(answers, 'm02a') === 'yes' ? 20 : 0;
  _score += answer(answers, 'm02b') === 'yes' ? 15 : 0;
  _score += answer(answers, 'm02c') === 'yes' ? 15 : 0;
  if (_score > pre) _score += adv * 2; // Not sure why M2 is special but sure

  // Score 3
  pre = _score;
  _score += answer(answers, 'm03a') === 'yes' ? 10 : 0;
  if (_score > pre) _score += adv;
  ...
  pre = _score;
  _score +=
    Number(answer(answers, 'm05a')) * 10 + Number(answer(answers, 'm05b')) * 15;
  if (_score > pre) _score += adv;

  // Score 6
  pre = _score;
  _score += answer(answers, 'm06a') === 'yes' ? 10 : 0;
  if (_score > pre) _score += adv;
  ...
  pre = _score;
  _score +=
    answer(answers, 'm09a') === 'yes'
      ? Number(answer(answers, 'm09b')) * 10
      : 0;
  if (_score > pre) _score += adv;

  // Score 10
  pre = _score;
  _score += answer(answers, 'm10a') === 'yes' ? 20 : 0;
  if (_score > pre) _score += adv;

  // Score 11
  pre = _score;
  const answer11a = answer(answers, 'm11a');
  const answer11b = answer(answers, 'm11b');
  _score +=
    answer11a === 'yes'
      ? answer11b === 'partly'
        ? 10
        : answer11b === 'completely'
        ? 15
        : 0
      : 0;
  if (_score > pre) _score += adv;
  ...
  pre = _score;
  _score +=
    Number(answer(answers, 'm12a')) * 10 + Number(answer(answers, 'm12b')) * 5;
  if (_score > pre) _score += adv;

  // Score 13
  pre = _score;
  _score += Number(answer(answers, 'm13a')) * 10;
  if (_score > pre) _score += adv;

  // Score 14
  switch (Number(answer(answers, 'm14a'))) {
    case 6:
      _score += 60;
      break;
    case 5:
